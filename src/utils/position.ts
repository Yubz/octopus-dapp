...
import { Position, PositionHistory } from "../models/ekubo/api";
import { PositionUi } from "../models/ui/position-ui";
import {
  GetTokenInfoRequest,
  GetTokenInfoResult,
} from "../models/ekubo/smartcontract";

export const getPositionsUi = async (
  positions: Array<Position>
): Promise<Array<PositionUi>> => {
...
export const getPositions = async (
  ...
  return (
    await fetch("https://mainnet-api.ekubo.org/positions/" + address).then(
      (res) => res.json()
    )
  ).data;
};

const getPositionsInfos = async (
  params: Array<GetTokenInfoRequest>
): Promise<Array<GetTokenInfoResult>> => {
...
const getPositionUi = async (
  position: Position,
  positionInfos: GetTokenInfoResult,
  positionHistory: PositionHistory,
  tokens: Array<Token>
): Promise<PositionUi> => {
  const token0 = tokens.find(
    (token) =>
      token.l2_token_address.slice(-10) === position.pool_key.token0.slice(-10)
  ) as Token;
  const token1 = tokens.find(
    (token) =>
      token.l2_token_address.slice(-10) === position.pool_key.token1.slice(-10)
  ) as Token;
  const currentPrice = sqrtRatioToPrice(
    Number(positionInfos.pool_price.sqrt_ratio)
  );
  ...
  const initialAmount0 =
    Number(positionHistory.events[0].delta0) / 10 ** token0.decimals;
  const initialAmount1 =
    Number(positionHistory.events[0].delta1) / 10 ** token1.decimals;
  ...
  const totalInitialAmount =
    initialAmount0 * token0Price + initialAmount1 * token1Price;
  ...
  const durationPositionInDays = daysBetweenDates(
    new Date(positionHistory.events[0].timestamp),
    new Date()
  );
  ...
  return {
    ...
    totalPnl: totalPnl,
    totalApr: calculateSimpleAPR(
      totalPnl / totalInitialAmount,
      durationPositionInDays
    ),
    durationPositionInDays: durationPositionInDays,
    feeApr: calculateSimpleAPR(
      totalFeesAmount / totalInitialAmount,
      durationPositionInDays
    ),
  };
};
