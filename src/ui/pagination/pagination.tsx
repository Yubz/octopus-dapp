...
const Pagination: React.FC<PaginationProps> = ({ currentPage, totalItems, itemsPerPage, onPageChange }) => {
	...
	const displayButtons = (currentPage: number): Array<JSX.Element> => {
		...
		if (currentPage < 3 && numberOfPages() > 5) {
			...
			buttons = ['...', (numberOfPages() - 3).toString(), (numberOfPages() - 2).toString(), (numberOfPages() - 1).toString()];
		} else {
			buttons = [
				'...',
	...
	return (
		<>
			<IconButton key="arrow-left" variant="outlined" disabled={page === 0} onClick={() => handlePageChange(page - 1)}>
				<KeyboardArrowLeft />
			</IconButton>
			<IconButton key="first" variant="outlined" color={page === 0 ? 'primary' : 'neutral'} onClick={() => handlePageChange(0)}>
				1
			</IconButton>
			{buttons}
			<IconButton key="arrow-right" variant="outlined" disabled={totalItems !== -1 ? page >= numberOfPages() - 1 : false} onClick={() => handlePageChange(page + 1)}>
				<KeyboardArrowRight />
			</IconButton>
		</>
	);
